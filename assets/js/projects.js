...
function renderProjectCard(project, isOngoing) {
    ...
    // Create a unique project ID from title
    const projectId = project.title.toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '');
    const categoryId = project.category.toLowerCase().replace(/\s+/g, '_');
    
    return `
        <div class="relative group mb-8" data-project-id="${projectId}" data-category="${categoryId}">
            <!-- Timeline dot -->
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200 dark:border-slate-700 hover:border-primary dark:hover:border-primary transition-all duration-200 cursor-pointer shadow-sm hover:shadow-md">
                ...
                <div class="flex items-center justify-between gap-2 pt-3 border-t border-slate-100 dark:border-slate-700">
                    ...
                    ${project.link ? `
                        <a href="${project.link}" target="_blank" rel="noopener noreferrer" data-link-type="project" class="inline-flex items-center gap-1 text-xs font-medium text-primary hover:text-blue-700 dark:hover:text-blue-400 transition-colors duration-200">
