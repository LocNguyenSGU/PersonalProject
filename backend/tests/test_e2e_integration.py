...
async def test_user_segmentation_flow(async_session, mock_llm_service):
    ...
    events = [
        ...
        for i, event_name in enumerate([
            "project_click", "project_click", "skill_hover", "deep_read", "section_view"
        ])
    ]
    ...
    assert segment.segment in ["ML_ENGINEER", "FULLSTACK_DEV", "RECRUITER", "STUDENT", "CASUAL"]
...
async def test_rules_generation_flow(async_session, mock_llm_service):
    ...
    segment = UserSegment(
        ...
        xai_explanation={"what": "test", "why": "test", "so_what": "test", "recommendation": "test"},
        event_summary={},
...
async def test_hourly_analysis_job(async_session, mock_llm_service):
    ...
    stmt = select(UserSegment).where(
        UserSegment.user_pseudo_id.in_(users)
    )
    result = await async_session.execute(stmt)
