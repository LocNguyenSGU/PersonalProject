...
class GA4Service:
    ...
    async def fetch_events(self, hours: int = 1) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for row in response.rows:
                ...
                event_timestamp = int(row.dimension_values[2].value) // 1000000  # Convert micros to seconds
                
                # Fetch event parameters (requires separate query per event)
                event_params = await self._fetch_event_params(event_name, user_pseudo_id, event_timestamp)
    ...
    async def _fetch_event_params(self, event_name: str, user_pseudo_id: str, event_timestamp: int) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            request = RunReportRequest(
                property=f"properties/{self.property_id}",
                date_ranges=[DateRange(
                    start_date=datetime.fromtimestamp(event_timestamp).strftime("%Y-%m-%d"),
                    end_date=datetime.fromtimestamp(event_timestamp).strftime("%Y-%m-%d")
                )],
                dimensions=[
            ...
            if response.rows:
                ...
                for i, dim in enumerate(row.dimension_values):
                    if dim.value and dim.value != "(not set)":
                        param_name = request.dimensions[i].name.replace("customEvent:", "")
