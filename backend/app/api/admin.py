...
async def login(req: Request, request: LoginRequest):
    ...
    try:
        ...
        return LoginResponse(
            access_token=access_token, token_type="bearer", expires_in=28800
        )
...
async def get_segments():
    ...
    try:
        ...
        async with get_async_session() as session:
            ...
            return {"total_users": total_users, "distribution": distribution}
    except Exception as e:
...
async def get_events(hours: int = 24):
    ...
    try:
        ...
        async with get_async_session() as session:
            ...
            top_stmt = (
                select(
                    AnalyticsRaw.event_name, func.count(AnalyticsRaw.id).label("count")
                )
                .where(AnalyticsRaw.created_at > since)
                .group_by(AnalyticsRaw.event_name)
                .order_by(func.count(AnalyticsRaw.id).desc())
            )
...
async def get_user_events(user_pseudo_id: str, limit: int = 50):
    ...
    try:
        ...
        async with get_async_session() as session:
            stmt = (
                select(AnalyticsRaw)
                .where(AnalyticsRaw.user_pseudo_id == user_pseudo_id)
                .order_by(AnalyticsRaw.created_at.desc())
                .limit(limit)
            )
...
async def get_event_types():
    ...
    try:
        ...
        async with get_async_session() as session:
            stmt = select(distinct(AnalyticsRaw.event_name)).order_by(
...
async def get_rules():
    ...
    try:
        ...
        async with get_async_session() as session:
            ...
            return {"total_rules": total_rules, "rules": rules_data}
    except Exception as e:
...
async def get_insights():
    ...
    try:
        ...
        async with get_async_session() as session:
            ...
            stmt = (
                select(UserSegment)
                .where(UserSegment.xai_explanation.isnot(None))
                .order_by(UserSegment.analyzed_at.desc())
                .limit(10)
            )
                ...
                insights.append(
                    {
                        ...
                        "analyzed_at": (
                            segment.analyzed_at.isoformat()
                            if segment.analyzed_at
                            else None
                        ),
                    }
                )
    ...
    try:
        ...
        async with get_async_session() as session:
            ...
            if existing_rule:
                ...
                existing_rule.reasoning = (
                    request.reasoning
                    or f"Manual override at {datetime.utcnow().isoformat()}"
                )
                ...
                new_rule = PersonalizationRules(
                    ...
                    or f"Manual creation at {datetime.utcnow().isoformat()}",
                    xai_explanation={
    ...
    try:
        ...
        async with get_async_session() as session:
            ...
            if not existing_rule:
                raise HTTPException(
                    status_code=404, detail=f"No rule found for segment {segment}"
                )
